...
```bash
.sp/bin/python src/orchestrator.py
```
...
* **Missing Cython Compiled Modules**: Cython routing fallback flags system status as using unoptimized modules. Fix: Compile the module with the `.sp` environment python before starting the dashboard.
