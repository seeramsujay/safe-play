...
5. Start the async Python orchestrator middleware script:
```bash
python src/orchestrator.py --config config/schema.json
```

6. Simulate an anomalous telemetry payload ($>3.0 \text{ people/m}^2$) via a lightweight local curl request or MQTT pub call to trigger the Human-In-The-Loop (HITL) 2-second SLA timer window.
